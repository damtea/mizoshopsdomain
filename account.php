...
$query="SELECT * FROM customer where CUSTOMER_ID='".$_SESSION['customer_id']."'";
 ...
 while($customer=mysqli_fetch_assoc($result))
 {
     ...
     $cpassword=$customer['CUSTOMER_PASSWORD'];
     $ccontact=$customer['CUSTOMER_CONTACT'];
...
<script>
   ...
   document.getElementById('cemail').disabled = true;
    document.getElementById('ccontact').disabled = true;
};
    function activename()
    {
        document.all.cname.disabled=false;
        document.all.cancel.hidden=false;
        document.all.editname.hidden=true;
        document.all.savenames.hidden=false;
        
        
    }
    function activenamecancel()
    {
        document.all.cname.disabled=true;
        document.all.editname.hidden=false;
        document.all.cancel.hidden=true;
        document.all.savenames.hidden=true;
    }
    function activeemail()
    {
        document.all.cemail.disabled=false;
        document.all.cancelemail.hidden=false;
        document.all.editemail.hidden=true;
        document.all.saveemails.hidden=false;
        
    }
    function activeemailcancel()
    {
        document.all.cemail.disabled=true;
        document.all.cancelemail.hidden=true;
        document.all.editemail.hidden=false;
        document.all.saveemails.hidden=true;
        
    }
     function activecontact()
    {
        document.all.ccontact.disabled=false;
        document.all.cancelcontact.hidden=false;
        document.all.contactedit.hidden=true;
        document.all.savephones.hidden=false;
        
    }
    function activecontactcancel()
    {
        document.all.ccontact.disabled=true;
        document.all.cancelcontact.hidden=true;
        document.all.contactedit.hidden=false;
        document.all.savephones.hidden=true;
...
function savephone()
{
    ...
    var contact=document.getElementById('ccontact').value;
        ...
        if(contact==="")
        {
            alert("Phone number chhu phawt rawh!");
        }
        else
        {
        ...
        <div class="col-md-12" >
         ...
         <ul id="sideManu" class="nav nav-tabs nav-stacked">
              <li class="subMenu open w-100">
                ...
                <ul>
                  <li>
                      <a href="account" class="active"><i class="icon-chevron-right"></i>Profile Information </a>
                  </li>
                  <li>
                    <a href="manageaddress"><i class="icon-chevron-right"></i>Manage Addresses </a>
                  </li>
                  <li>
          ...
          <div class="col-md-8 offset-md-2">
            <div class="row">
                    ...
                    <table>
                        <tr>
                            <td><h5> Personal Information </h5> <label id="nmsg"></label></td>
                            <td> &nbsp;&nbsp;&nbsp; <button class="btn btn-link" id='editname' onclick="activename();">[edit]</button> <button class="btn btn-link" id='cancel' hidden="hidden" onclick="activenamecancel();">[cancel]</button> </td>
                       <td> &nbsp;&nbsp;&nbsp; <button class="btn btn-primary" id='savenames' hidden="hidden" onclick="savename();">SAVE</button>  </td>
                        </tr>
                        
                    </table>
                    <input type="text" id='cname' class="form-control" value="<?php echo $cname; ?>">
                     <br /><br />
                     <table>
                        <tr>
                            <td><h5> Email Address </h5> <label id="emsg"></label></td>
                            <td> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <button class="btn btn-link" id='editemail' onclick="activeemail();">[edit]</button><button class="btn btn-link" hidden="hidden" id='cancelemail' onclick="activeemailcancel();">[cancel]</button></td>
                            <td><button class="btn btn-link" onclick="deactivename();">[Change Password]</button></td>
                            <td> &nbsp;&nbsp;&nbsp; <button class="btn btn-primary" id='saveemails' hidden="hidden" onclick="saveemail();">SAVE</button>  </td>
                        </tr>
                        
                    </table>
                     <input type="email" id='cemail' class="form-control" value="<?php echo $cemail; ?>">
                     <br /><br />
                      <table>
                        <tr>
                            <td><h5> Mobile Number </h5><label id="pmsg"></label></td>
                            <td> &nbsp;&nbsp;&nbsp; <button class="btn btn-link" id="contactedit" onclick="activecontact();">[edit]</button><button class="btn btn-link" id="cancelcontact" hidden="hidden" onclick="activecontactcancel();">[cancel]</button></td>
                            <td> &nbsp;&nbsp;&nbsp; <button class="btn btn-primary" id='savephones' hidden="hidden" onclick="savephone();">SAVE</button>  </td>
                        </tr>
                        
                    </table>
                     <input type="text" id='ccontact' class="form-control" value="<?php echo $ccontact; ?>" />
                     
                     <form action="confirmdeactivate" method="POST">
                         <br /><br /><br /><br /> 
                     <button class="btn btn-link" type="submit">[Deactivate my account]</button>
                     </form>
